...
class CloudinaryService extends GetxService {
  ...
  Future<String?> uploadDocument(File file, {String? folder}) async {
    try {
      ...
      // Endpoint 'raw/upload' dùng cho các tệp không phải hình ảnh/video (PDF, DOCX, ZIP...)
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        final Map<String, dynamic> errorData = json.decode(response.body);
        final errorMessage = errorData['error']?['message'] ?? 'Unknown error';
        throw Exception('Cloudinary Raw Upload Failed: $errorMessage (Status: ${response.statusCode})');
      }
    } catch (e) {
      Get.log('CloudinaryService uploadDocument error: $e');
      rethrow;
    }
  }
}
